...
import { Product, GoodsOut } from '../../../../core/models';
...
export class CreateGoodsOutDialog implements OnInit {
  ...
  addItem(): void {
    const itemGroup = this.fb.group({
      ...
      requestedQuantity: ['', [Validators.required, Validators.min(0.01)]],
      note: ['']
    });
  ...
  onSave(): void {
    ...
    const goodsOut: Partial<GoodsOut> = {
    ...
    this.goodsOutService.createGoodsOut(goodsOut).subscribe({
      next: () => {
